...
from scripts.primary_funcs.basic_nn_units import Add
from scripts.structure.Net_template import Net_template

__author__ = 'Lou Zehua'
__time__ = '2019/7/29 1:02'


class Net_add(Net_template):
...
if __name__ == '__main__':
    # input
    N = 100
    x_input_array = np.array(2 * torch.rand(N, 2) - 1)
    ...
    # save model
    whole_save_path = os.path.join(filePathConf.absPathDict[filePathConf.MODELS_WHOLE_NET_PARAMS_DIR], training_purpose, 'Net_add.model')
    state_dict_save_path = os.path.join(filePathConf.absPathDict[filePathConf.MODELS_STATE_DICT_DIR], training_purpose, 'Net_add.state_dict')
